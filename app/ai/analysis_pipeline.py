...
from app.ai.models.roof_detector import RoofDetector # Import the facade RoofDetector
...
class RoofAnalysisPipeline:
    ...
    def analyze_image(
        ...
        **kwargs
        ...
        if isinstance(model, AbstractSegmentationModel):
        ...
        elif isinstance(model, RoofDetector): # Handle the facade RoofDetector
            # The RoofDetector facade will return DetectionResults for planes, lines, features
            raw_ai_results = model.detect(image, **kwargs)
            ...
            if all(isinstance(res, DetectionResult) for res in raw_ai_results):
                ...
                logger.warning("RoofDetector returned mixed or unsupported AI result types. Returning empty results.")
                raise TypeError("Unsupported or mixed AI result types from RoofDetector for geometry conversion.")

        else: # Generic CoreAIPipeline for other AI models (e.g., direct geometry prediction)
