...
def _multi_plane_ransac(points, distance_threshold=0.06, min_inliers=200, max_planes=15,
    ...
    SPLIT_MIN_FRACTION = 0.03  # min 3% bodov zhluku musi byt, aby bol signifikantny
    SPLIT_MIN_POINTS = 20      # min 20 bodov (male plochy sa tiez rozdelia)
    split_planes = []
    for p in raw_planes:
    ...
    if densities:
        ...
        # Ploche roviny maju prirodzene vyssie density (menej bodov na vacsej
        # ploche kvoli kolmemu dopadu lasera), preto pre ne pouzijeme vyssi faktor.
        # Ale EXTREMNE riedke ploche roviny (roztrusene body z rims/parapetov
        # na rovnakej vyske) su artefakty - tie vyradime tiez.
        flat_factor = density_outlier_factor * 4.0
        for p in raw_planes:
            if p.get("_is_flat"):
                p["_low_confidence"] = p["_density"] > med * flat_factor
            else:
