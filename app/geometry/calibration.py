...
class CalibrationService:
    ...
    @staticmethod
    def convert_area_unit(value: float, from_unit: Literal["sq_mm", "sq_cm", "sq_m"], to_unit: Literal["sq_mm", "sq_cm", "sq_m"]) -> float:
        """
        Converts an area value from one real-world unit to another.

        Args:
            value (float): The area value to convert.
            from_unit (Literal["sq_mm", "sq_cm", "sq_m"]): The unit of the input area.
            to_unit (Literal["sq_mm", "sq_cm", "sq_m"]): The target unit for conversion.

        Returns:
            float: The converted area value.
        """
        conversion_factors = {
            "sq_mm": {"sq_mm": 1, "sq_cm": 0.01, "sq_m": 0.000001},
            "sq_cm": {"sq_mm": 100, "sq_cm": 1, "sq_m": 0.0001},
            "sq_m": {"sq_mm": 1000000, "sq_cm": 10000, "sq_m": 1},
        }
        if from_unit not in conversion_factors or to_unit not in conversion_factors[from_unit]:
            raise ValueError(f"Unsupported area unit conversion: from {from_unit} to {to_unit}")
